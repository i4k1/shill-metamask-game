if ( typeof window.ethereum !== undefined && window.ethereum.isMetaMask ) {
    ...
    changeChainIDButton.addEventListener(`click`, async () => {
        await window.ethereum.request({
                method: `wallet_switchEthereumChain`,
                params: [{ chainId: `0x${Number(text_input_chainid.value).toString(16)}` }]
        });
        await getAccount();
    });

    ethereumButton.addEventListener(`click`, () => { getAccount(); });
    showAccount.innerHTML = `<b>Connect Wallet</b>`;
    ...
    async function getAccount() {
        ...
        showAccount.innerHTML = `<code>${account}</code>`;
    ...
    selectEthMainnet.addEventListener(`click`, async () => {
        await window.ethereum.request({
            method: `wallet_switchEthereumChain`,
            params: [{ chainId: `0x1` }]
        });
    ...
    selectBnbTestnet.addEventListener(`click`, async () => {
        await window.ethereum.request({
            method: `wallet_switchEthereumChain`,
            params: [{ chainId: `0x61` }]
        });
    ...
    selectPolygonMumbai.addEventListener(`click`, async () => {
        await window.ethereum.request({
            method: `wallet_switchEthereumChain`,
    ...
    const selectArbitrumGoerli = document.querySelector(`.selectArbitrumGoerli`);
    selectArbitrumGoerli.addEventListener(`click`, async () => {
        await window.ethereum.request({
            method: `wallet_switchEthereumChain`,
